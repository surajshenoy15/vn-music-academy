        ...
        <div className="mb-6">
          <div className="flex items-start justify-between mb-4 gap-2 flex-wrap">
            <div className="flex items-center gap-3 min-w-0">
              <div className="p-2 md:p-3 rounded-xl shadow-lg bg-white shrink-0">
                <Users className="w-6 h-6 md:w-8 md:h-8" style={{ color: '#4A4947' }} />
              </div>
              <div className="min-w-0">
                <h1 className="text-xl md:text-3xl font-bold" style={{ color: '#4A4947' }}>Students Directory</h1>
                <p className="text-gray-600 text-sm">Manage and view all registered students</p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-green-50 px-3 py-2 rounded-lg border border-green-200 shrink-0">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
              <span className="text-xs md:text-sm text-green-700 font-medium">Live Updates</span>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="relative w-full">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-opacity-50 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white text-sm"
                />
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    <div className="text-xl md:text-2xl font-bold" style={{ color: '#4A4947' }}>{filteredStudents.length}</div>
                    <div className="text-xs text-gray-600">Students Found</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl md:text-2xl font-bold" style={{ color: '#4A4947' }}>{students.length}</div>
                ...
                <button
                  onClick={handleRefresh}
                  disabled={refreshing}
                  className="p-3 rounded-xl text-white transition-all duration-200 hover:shadow-lg disabled:opacity-70"
                  style={{ backgroundColor: '#4A4947' }}
                >
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              ...
              <table className="w-full">
                <thead style={{ backgroundColor: '#4A4947' }}>
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-white font-semibold">
                      <div className="flex items-center space-x-2"><Hash className="w-4 h-4" /><span>Student ID</span></div>
                    </th>
                    <th className="px-6 py-4 text-left text-white font-semibold">
                      <div className="flex items-center space-x-2"><Users className="w-4 h-4" /><span>Name</span></div>
                    </th>
                    <th className="px-6 py-4 text-left text-white font-semibold">
                    ...
                    <th className="px-6 py-4 text-left text-white font-semibold">
                      <div className="flex items-center space-x-2"><BookOpen className="w-4 h-4" /><span>Course</span></div>
                    </th>
                    <th className="px-6 py-4 text-left text-white font-semibold">
                ...
                <tbody>
                  {filteredStudents.map((student, index) => (
                    <tr
                      key={student.id}
                      className={`border-b border-gray-100 hover:bg-gray-50 transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}
                    >
                      <td className="px-6 py-4">
                        <div className="relative group w-fit">
                          {student.profile_pic ? (
                            <img src={student.profile_pic} alt={student.name} className="w-12 h-12 rounded-full object-cover border-2 border-gray-200" />
                          ) : (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          {student.joined_at && (
                            <span className="text-xs text-gray-400 mt-1">Joined: {new Date(student.joined_at).toLocaleDateString()}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-semibold" style={{ color: '#4A4947' }}>{student.name}</div>
                      </td>
                      <td className="px-6 py-4"><div className="text-gray-700">{student.email}</div></td>
                      <td className="px-6 py-4"><div className="text-gray-700">{student.phone || '-'}</div></td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center space-x-2">
                          ...
                          <button onClick={() => handleEditClick(student)} className="p-2 rounded-lg bg-blue-600 text-white transition-all duration-200 hover:shadow-md hover:bg-blue-700" title="Edit Student">
                            <Edit className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDeleteClick(student)} className="p-2 rounded-lg bg-red-600 text-white transition-all duration-200 hover:shadow-md hover:bg-red-700" title="Delete Student">
            ...
            <div className="lg:hidden space-y-4 p-4">
              {filteredStudents.map((student) => (
                <div key={student.id} className="bg-gray-50 rounded-xl p-4 border border-gray-200 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-center gap-3 mb-4 w-full min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base font-semibold truncate" style={{ color: '#4A4947' }}>
                        {student.name}
                      </h3>
                      <p className="text-xs text-gray-500 truncate">ID: {student.id.slice(0, 8)}...</p>
                      {student.joined_at && (
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="text" name="name" value={editFormData.name} onChange={handleEditChange}
                    className="w-full p-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2" required />
                </div>
                <div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="relative w-full px-3 py-3 text-left bg-white border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-2"
                  >
                    <span className={editFormData.course ? "text-gray-900" : "text-gray-400"}>
                      {editFormData.course || "Select a course"}
                    </span>
                    <ChevronDown className={`absolute right-3 top-3 h-5 w-5 text-gray-400 transform transition-transform duration-200 ${isDropdownOpen ? "rotate-180" : ""}`} />
                  </button>
                  {isDropdownOpen && (
                    <div className="absolute left-0 right-0 z-50 mt-2 bg-white border border-gray-200 rounded-lg shadow-xl">
                      {["Guitar", "Piano or Keyboard", "Music Production"].map((course, index) => (
                        <button
                          key={index}
                          type="button"
                          onClick={() => { setEditFormData((prev) => ({ ...prev, course })); setIsDropdownOpen(false); }}
                ...
                <div className="flex space-x-3 pt-4">
                  ...
                  <button type="submit" disabled={isEditing}
                    ...
                    style={{ backgroundColor: '#4A4947' }}>
                    {isEditing ? (<><Loader2 className="w-4 h-4 animate-spin mr-2" />Saving...</>) : (<><Check className="w-4 h-4 mr-2" />Save Changes</>)}
                  </button>
                </div>
              </form>
            </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => { setUploadingProfilePic(null); setProfilePicFile(null); }}
              ...
              <div className="text-center mb-6">
                <div className="mx-auto w-24 h-24 rounded-full mb-4 overflow-hidden border-4 border-gray-200">
                  {uploadingProfilePic.profile_pic ? (
                    <img src={uploadingProfilePic.profile_pic} alt={uploadingProfilePic.name} className="w-full h-full object-cover" />
                  ) : (
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Choose new profile picture</label>
                  <input type="file" accept="image/*" onChange={handleProfilePicChange}
                    className="w-full p-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2" />
                </div>
                {profilePicFile && (
                  <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                    <p className="text-sm text-green-800">Selected: {profilePicFile.name}</p>
                  </div>
                )}
                <div className="flex space-x-3">
                  {uploadingProfilePic.profile_pic && (
                    <button onClick={() => handleRemoveProfilePic(uploadingProfilePic)}
                      className="flex-1 py-3 border border-red-300 text-red-600 rounded-lg hover:bg-red-50">
                      Remove Photo
                    </button>
                  )}
                  <button onClick={handleProfilePicUpload} disabled={!profilePicFile}
